...
# p probability
tk.Label(left_frame, text="Probability P:", font=font).grid(
    row=0, column=0, sticky=tk.E, padx=(0, 10)
)
probability_entry = tk.Entry(left_frame, width=5, font=font)
probability_entry.grid(row=0, column=1, sticky=tk.W, padx=(10, 0))

# U
tk.Label(left_frame, text="U:", font=font).grid(
    row=1, column=0, sticky=tk.E, padx=(0, 10)
)
u_entry = tk.Entry(left_frame, width=5, font=font)
u_entry.grid(row=1, column=1, sticky=tk.W, padx=(10, 0))


# label to show result of RP
label_rp = tk.Label(left_frame, text="", foreground="#00b4d9", font=font)
label_rp.grid(row=2, column=1, sticky=tk.W, padx=(10, 0))

# S - Number of samples
tk.Label(left_frame, text="S:", font=font).grid(
    row=3, column=0, sticky=tk.E, padx=(0, 10)
)
s_entry = tk.Entry(left_frame, width=5, font=font)
s_entry.grid(row=3, column=1, sticky=tk.W, padx=(10, 0))


# label to show result of monte carlo simulation
label_test = tk.Label(left_frame, text="", foreground="#00b4d9", font=font)
label_test.grid(row=4, column=1, sticky=tk.W, padx=(10, 0))

frame = tk.Frame(left_frame)
frame.grid(row=5, column=1, sticky=tk.W)
# number of vertices
tk.Label(left_frame, text="Enter number of vertices:", font=font).grid(
    row=5, column=0, sticky=tk.E, padx=(0, 10)
)
...
# number of edges
tk.Label(left_frame, text="Enter number of edges:", font=font).grid(
    row=6, column=0, sticky=tk.E, padx=(0, 10)
)
num_edges_entry = tk.Entry(left_frame, width=5, font=font)
...
def update_graph(event=None):
    ...
    a.cla()
    nx.draw_networkx(
        G, pos=nx.circular_layout(G), ax=a, with_labels=True, node_color="#00b4d9"
    )
    a.set_xlim(xlim)
    a.set_ylim(ylim)
    a.axis("off")
...
# RP button
buttonCalculateRP = tk.Button(
    left_frame, text="Compute RP", command=compute_rp, state=tk.DISABLED, font=font
)
